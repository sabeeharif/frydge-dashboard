...
export default function RoutesNewPage() {
    ...
    const refetchVenueGroups = async () => {
        try {
            ...
            if (response.ok) {
                ...
                console.log('=== VENUE GROUPS REFETCH DEBUG ===')
                console.log('Raw API data:', data)
                console.log('Transformed groups:', groupsWithExpansion)
                groupsWithExpansion.forEach(group => {
                    console.log(`Group ${group.name || group.id}:`, {
                        id: group.id,
                        name: group.name,
                        userId: group.userId,
                        userIdType: typeof group.userId,
                        userIdLength: group.userId?.length
                    })
                })
                console.log('=== END VENUE GROUPS REFETCH DEBUG ===')
    ...
    const getGroupTypeColor = (group) => {
        ...
        console.log(`Color coding for group ${group?.name || group?.id}:`, {
            group: group,
            userId: group?.userId,
        ...
        if (uniqueCount === 0) {
            // Not assigned to any driver - white
            console.log(`Group ${group?.name || group?.id} -> WHITE (0 users)`)
            return "bg-white border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-gray-400"
        } else if (uniqueCount === 1) {
            // Assigned to exactly 1 unique driver - green
            console.log(`Group ${group?.name || group?.id} -> GREEN (1 user)`)
            return "bg-green-100 border-green-400 text-green-800 hover:bg-green-200 hover:border-green-500"
        } else {
            // Assigned to 2 or more different drivers - red (conflict)
            console.log(`Group ${group?.name || group?.id} -> RED (${uniqueCount} users)`)
            return "bg-red-100 border-red-400 text-red-800 hover:bg-red-200 hover:border-red-500"
        }
    }
    ...
    const handleModalVenueDrop = (e, targetIndex) => {
        ...
        if (draggedItem) {
            ...
            if (fromIndex !== -1) {
                ...
                setSelectedVenues(prev => {
                    // If somehow duplicate slipped in, keep prev
    ...
    const createRouteForDriver = async (driver, venues, groupInfo = null) => {
        ...
        try {
            ...
            if (groupInfo) {
                ...
                try {
                    const existingResponse = await api.getDriverRoutes({ userId: driver.userId || driver.id, fetchAll: true })
                ...
                if (response.ok) {
                    ...
                    if (routes.length > 0) {
                        ...
                        if (updateResponse.ok) {
                            ...
                            try {
                                console.log('=== VENUE GROUP ASSIGNMENT DEBUG ===')
                                ...
                                console.log('Sending venue group assignment update:', groupUpdateData)
                                const groupUpdateResponse = await api.updateVenueGroup(groupUpdateData)
                                console.log('Venue group assignment response:', groupUpdateResponse)
                                
                                if (groupUpdateResponse.ok) {
                                    const responseData = await groupUpdateResponse.json().catch(() => ({}))
                                    console.log('Venue group assignment updated successfully:', responseData)
                                } else {
                                    console.error('Failed to update venue group during assignment:', groupUpdateResponse)
                                    const errorData = await groupUpdateResponse.json().catch(() => ({}))
                                    console.error('Venue group assignment error details:', errorData)
                                }
                                console.log('=== END VENUE GROUP ASSIGNMENT DEBUG ===')
                            } catch (groupError) {
                                console.error('Error updating venue group during assignment:', groupError)
                            }
    ...
    const handleDropOnVenueList = async (e) => {
        ...
        if (draggedFromDriver) {
            ...
            if (driver) {
                ...
                const remainingGroups = driver.assignedVenues
                ...
                const { updatedVenues, updatedGroups } = recalculateAllPriorities(
                    ...
                    remainingGroups.map(g => ({ 
                        groupId: g.id, 
                        priority: g.priority, 
                        userId: Array.isArray(g.userId) ? g.userId : [driver.userId || driver.id]
                    }))
                )
                
                // Combine updated items and sort by priority
                ...
                setDrivers(prev =>
                    prev.map(d =>
                        d.id === draggedFromDriver
                            ? {
                                ...d,
                                assignedVenues: allRemainingItems
                            }
                            : d
                    )
                )
                        ...
                        if (routes.length > 0) {
                            ...
                            const remainingGroups = (route.venueGroupsInfo || [])
                                ...
                                .map((g, index) => ({ 
                                    ...g, 
                                    priority: index + 1,
                                    userId: g.userId || [driver.userId || driver.id] // Preserve existing userId or use current driver's userId
                                }))
    ...
    const handleReorderVenues = async (driverId, fromIndex, toIndex) => {
        ...
        const groups = driver.assignedVenues.filter(item => item.type === 'group').map(g => ({
            groupId: g.id,
            priority: g.priority,
            userId: g.userId || [driver.userId || driver.id] // Ensure userId is always an array
        }))
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (routes.length > 0) {
                    ...
                    if (updateResponse.ok) {
                        success(`Item priorities updated for ${driver.name}!`)
            ...
            <div className="mb-8">
                <div className="flex items-center justify-between">
                    ...
                    >
                        ...
                        <Link
                            href="/dashboard/driver-routes/manage-driver-venue-group"
                                        ...
                                        <div key={group.id} className="border-2 border-slate-200 rounded-lg overflow-hidden">
                                            ...
                                            {isExpanded && (
                                                <div className="p-3 space-y-2 bg-white border-t border-slate-200">
                                                    {group.venues && group.venues.map((venue, venueIndex) => (
                                                        <div
                                                            key={`${group.id}-venue-${venue.id || venue.venueId}-${venueIndex}`}
                                                                        ...
                                                                        <div className="p-3 space-y-2 bg-white border-t border-purple-200">
                                                                            {item.venues && item.venues.length > 0 ? (
                                                                                item.venues.map((venue, venueIndex) => (
                                                                                    <div
                                                                                        key={`${item.id}-venue-${venue.id || venue.venueId}-${venueIndex}`}
                                                                                        className="p-2 text-sm text-slate-600 bg-slate-50 rounded border border-slate-200"
                                                                                    >
                                                                                        <div className="flex items-center gap-2">
                                                        ...
                                                        <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-4 border-b border-gray-200">
                                                            <div className="flex items-center justify-between">
                                                                ...
                                                                <div className="flex gap-2">
                                                                    <Link
                                                                        href={`/dashboard/driver-routes/manage-driver-venue-group?groupId=${group.groupId || group.id}`}
